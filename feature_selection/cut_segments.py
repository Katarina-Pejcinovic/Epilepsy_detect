...
# The function cuts segments of 10 minutes (150,000 samples) for all channels from each 2D np array
# Outputs:
# - a list 'called result_4d' of 4 3D np arrays, where each 3D np array is of size (s, x, 150000) where s is the number of 10-minute segments
# - a list 'label_result' of 4 1D numpy arrays where each 1D numpy array of size (s,) has the label for each 10-minute segment in the 3D np arrays in result_4d
# - a list 'patientID_result' of 4 1D numpy arrays where each 1D numpy array of size (s,) has the patientID for each 10-minute segment in the 3D np arrays in result_4d
    ...
    for idx, input_arrays in enumerate(data_list):
        ...
        for i, array in enumerate(input_arrays):
            ...
            num_segments_array = signal_length // 150000

            if num_segments_array >= 1:
                segments_array = np.array_split(array[:, :num_segments_array * 150000], num_segments_array, axis=1)
