...
# def get_features(list_signals, waveletname, lvl=5):
#     list_features = []
...
def get_features(data_path):
   all_features = np.zeros(shape=(2, 16, 21))
   i=0
   # iterate over files in data_path
   for filename in (os.listdir(data_path)):
      ...
      if os.path.isfile(f):
          list_signals = np.load(f)
          list_features = []
          for s in (range(16)):
              time_features = get_time_features(list_signals[s])
              freq_features = get_freq_features(list_signals[s])
              features = []
              for ff in freq_features:
                  features.append(ff)
              for tf in time_features:
                  features.append(tf)
              list_features.append(features)
      all_features[i] = np.array(list_features)
      i+=1
   return all_features
