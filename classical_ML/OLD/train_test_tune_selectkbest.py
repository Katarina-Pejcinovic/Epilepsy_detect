...
def create_svc_pipeline(stratified_kfold, scoring_method):
...
def create_rf_pipeline(stratified_kfold, scoring_method):
...
def train_test_tune_selectkbest(data, labels, patient_id, stratified_cv):
  ...
  for i, (train_idx, test_idx) in enumerate(stratified_cv):
    ...
    ##########################################
    svc_C = svc_best_params["svc__C"]
    svc_kernel = svc_best_params["svc__kernel"]
    svc_degree = svc_best_params["svc__degree"]
    selectkbest_k = svc_best_params["selectkbest__k"]
    ...
    scl = StandardScaler()
    X_train_svc_scale = scl.fit_transform(X_train)
    X_test_svc_scale = scl.transform(X_test)

    # Feature selection
    X_train_svc_feat = reducer.fit_transform(X_train_svc_scale, y_train)
    X_test_svc_feat = reducer.transform(X_test_svc_scale)

    # Train the model
    svm_model = SVC(kernel=svc_kernel, C=svc_C, gamma='auto', degree=svc_degree, random_state=42, probability=True)
    svm_model.fit(X_train_svc_feat, y_train)

    # Make predictions
    y_pred_test = svm_model.predict(X_test_svc_feat)
    ##########################################
    ...
    ##########################################
    rf_depth = rf_best_params["randomforestclassifier__max_depth"]
    rf_samples = rf_best_params["randomforestclassifier__min_samples_leaf"]
    rf_estimators = rf_best_params["randomforestclassifier__n_estimators"]
    selectkbest_k = rf_best_params["selectkbest__k"]
    ...
    # Turn data into z-scores
    scl = StandardScaler()
    X_train_rf_scale = scl.fit_transform(X_train)
    X_test_rf_scale = scl.transform(X_test)

    # Feature selection
    X_train_rf_feat = reducer.fit_transform(X_train_rf_scale, y_train)
    X_test_rf_feat = reducer.transform(X_test_rf_scale)

    # Train the model
    rf_model = RandomForestClassifier(max_depth=rf_depth, n_estimators=rf_estimators, min_samples_leaf=rf_samples)
    rf_model.fit(X_train_rf_feat, y_train)

    y_pred_test = rf_model.predict(X_test_rf_feat)
    ##########################################
    ...
    ##########################################
    xg_lr = xg_best_params["xgbclassifier__learning_rate"]
    xg_depth = xg_best_params["xgbclassifier__max_depth"]
    xg_estimators = xg_best_params["xgbclassifier__n_estimators"]
    selectkbest_k = xg_best_params["selectkbest__k"]
    ...
    # Turn data into z-scores
    scl = StandardScaler()
    X_train_xg_scale = scl.fit_transform(X_train)
    X_test_xg_scale = scl.transform(X_test)

    # Feature selection
    X_train_xg_feat = reducer.fit_transform(X_train_xg_scale, y_train)
    X_test_xg_feat = reducer.transform(X_test_xg_scale)

    # Train the model
    xg_model = XGBClassifier(max_depth=xg_depth, n_estimators=xg_estimators, learning_rate=xg_lr)
    xg_model.fit(X_train_xg_feat, y_train)

    y_pred_test = xg_model.predict(X_test_xg_feat)
    ##########################################
    ...
    ##########################################
    gmm_init = gmm_best_params["gaussianmixture__init_params"]
    gmm_cov = gmm_best_params["gaussianmixture__covariance_type"]
    selectkbest_k = gmm_best_params["selectkbest__k"]
    ...
    # Turn data into z-scores
    scl = StandardScaler()
    X_train_gmm_scale = scl.fit_transform(X_train)
    X_test_gmm_scale = scl.transform(X_test)

    # Feature selection
    X_train_gmm_feat = reducer.fit_transform(X_train_gmm_scale, y_train)
    X_test_gmm_feat = reducer.transform(X_test_gmm_scale)

    # Train the model
    gmm_model = GaussianMixture(n_components=2, init_params=gmm_init, covariance_type=gmm_cov)
    gmm_model.fit(X_train_gmm_feat, y_train)

    y_pred_test = gmm_model.predict(X_test_gmm_feat)
    ##########################################
  ...
  file = open('results/best_kbest_svc_params.txt','w')
  ...
  file = open('results/best_kbest_rf_params.txt','w')
  ...
  file = open('results/best_kbest_xg_params.txt','w')
  ...
  file = open('results/best_kbest_gmm_params.txt','w')
  ...
  with open('results/best_kbest_params_dict.pkl', 'wb') as f:
    pickle.dump(param_best, f)

  # Save best params scores to load later
  with open('results/best_kbest_scores.pkl', 'wb') as f:
    pickle.dump(param_scores, f)

  # Save all scores to load later
  with open('results/kbest_scores.pkl', 'wb') as f:
